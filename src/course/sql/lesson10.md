...
INSERT INTO orders (id, customer_id, product, amount, order_date) VALUES
  (1, 1, 'ノートPC', 98000, '2024-01-05'),
  (2, 1, 'マウス',    3500, '2024-01-10'),
  (3, 2, 'モニター', 42000, '2024-02-01');
```

### 問題1: INNER JOIN で注文一覧を取得

> 参照：[1. GROUP BY とは](#1-group-by-とは)

`customers` と `orders` を INNER JOIN して、顧客名・商品名・金額を取得してください。
...
**解説：** `INNER JOIN` は両テーブルで結合条件（`c.id = o.customer_id`）が一致する行のみ返します。佐藤（id=3）は注文がないためこの結果に含まれません。テーブルにエイリアスを付けると長いテーブル名を省略できます。

</details>

### 問題2: LEFT JOIN で注文なし顧客も含める

> 参照：[5. HAVING でグループに条件をつける](#5-having-でグループに条件をつける) ・ [6. WHERE vs HAVING の違い](#6-where-vs-having-の違い実行順序)

注文がない顧客も含めて全顧客を取得し、注文がない顧客の `product` は NULL として表示してください。
...
**解説：** `LEFT JOIN` は左テーブル（customers）の全行を保持します。対応する右テーブル（orders）の行がなければ NULL で埋めます。佐藤は注文がないので `product` と `amount` が NULL になります。

</details>

### 問題3: 顧客ごとの注文集計

> 参照：[9. 実務でよく使う集計パターン](#9-実務でよく使う集計パターン)

全顧客について、注文件数と合計金額を取得してください。注文がない顧客は件数 0・金額 0 として表示してください。
...
**解説：** LEFT JOIN の後に GROUP BY することで全顧客の集計ができます。`COUNT(o.id)` は o.id が NULL（=注文なし）の行を数えないため 0 になります。`SUM` が NULL になるケースは `COALESCE` で 0 に変換します。
