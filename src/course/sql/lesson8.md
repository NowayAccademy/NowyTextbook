...
INSERT INTO orders (id, customer_id, product, amount, status, order_date) VALUES
  ...
  (6, 3,  NULL,         5000, '完了',       '2024-02-10');
```

### 問題1: 件数と合計の集計

> 参照：[1. トランザクションとは](#1-トランザクションとは) ・ [2. BEGIN / COMMIT の基本](#2-begin-commit-の基本) ・ [3. ROLLBACK](#3-rollback取り消し)

`orders` テーブルの全件数と、`amount` の合計を取得してください。
...
**解説：** `COUNT(*)` は NULL を含む全行を数えます。`SUM(amount)` は amount の合計を返します。NULL が含まれていても `SUM` は NULL を無視して計算します。

</details>

### 問題2: ステータス別の件数

> 参照：[8. OFFSET ページングの問題](#8-offset-ページングの問題)

`status` ごとの注文件数と平均 `amount` を取得してください。平均は小数点以下を四捨五入してください。
...
**解説：** `GROUP BY status` で status ごとにグループを作り、各グループの `COUNT` と `AVG` を計算します。`ROUND` で小数点を四捨五入します。`AVG` も NULL を除いた値で計算されます。

</details>

### 問題3: NULL を除いた件数の比較

> 参照：[9. カーソルベースページングの概念](#9-カーソルベースページングキーセットページングの概念) ・ [10. カーソルベースページングの実装例](#10-カーソルベースページングの実装例)

`product` が NULL でない注文の件数と、全注文件数を同時に取得してください。
...
**解説：** `COUNT(*)` は NULL を含む全行を数えますが、`COUNT(カラム名)` はそのカラムが NULL の行をスキップします。id=6 の product が NULL なので `total_count=6`、`product_count=5` になります。この違いを意識することで NULL データの混入状況を把握できます。
