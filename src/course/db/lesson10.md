...
> 参照：[3. 論理削除のメリット・デメリット](#3-論理削除deletedat-isdeletedのメリット・デメリット)

`articles` テーブルに論理削除を実装してください。削除済みの記事は通常の参照クエリに含まれないようにしてください。
...
-- 削除済みも含めて全件参照（管理者用）
SELECT id, title, deleted_at FROM articles;
```

**解説：** `deleted_at` が NULL のものを「有効なデータ」として扱います。`deleted_at IS NOT NULL` で削除済みを判定できます。ほぼ全てのクエリで `WHERE deleted_at IS NULL` を付ける必要があるため、VIEW やアプリ層でのフィルタを統一すると漏れが減ります。

</details>

### 問題2: 履歴テーブルの設計

> 参照：[6. 履歴テーブルパターン](#6-履歴テーブルパターン)

`products` テーブルの変更履歴を保持する `products_history` テーブルを設計し、価格変更時に履歴を記録する手順を書いてください。
...
CREATE TABLE products_history (
  ...
  product_id  BIGINT NOT NULL,          -- 元テーブルへのFK（削除されても残せるよう参照整合性なし）
  name        TEXT NOT NULL,
  price       NUMERIC(10,2) NOT NULL,
  changed_at  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  changed_by  BIGINT,                   -- 変更したユーザーID
  operation   TEXT NOT NULL             -- 'UPDATE', 'DELETE' など
);
...
SELECT id, name, price, 'UPDATE'
FROM products
WHERE id = 10;

-- 2. 本テーブルを更新
UPDATE products SET price = 5800 WHERE id = 10;

COMMIT;
```

**解説：** 履歴テーブルはトランザクション内で「履歴INSERT → 本体UPDATE」の順で実行します。`products_history` の `product_id` には外部キー制約を付けないことが多いです（商品を削除しても履歴は残したいため）。

</details>

### 問題3: 削除戦略の選択

> 参照：[2. 物理削除のメリット・デメリット](#2-物理削除deleteのメリット・デメリット) ・ [7. 判断軸まとめ](#7-判断軸まとめどれを選ぶべきか)

以下のシナリオで物理削除・論理削除・履歴テーブルのどれを選ぶか答えてください。

1. チャットアプリのメッセージ削除（ユーザーが「削除」した場合、他ユーザーには「削除されました」と表示）
2. ECサイトの商品削除（一度も注文されていない商品のみ削除可能）
3. 人事システムの給与変更履歴（いつ誰がいくらだったか監査が必要）
...
| シナリオ | 推奨 | 理由 |
|----------|------|------|
| 1 | 論理削除 | 「削除されました」表示のために行は残す必要がある。`deleted_at` で管理 |
| 2 | 物理削除（外部キー制約で保護） | 注文がある商品は `ON DELETE RESTRICT` で物理削除を防ぐ。一度も注文されていなければ安全に削除可能 |
| 3 | 履歴テーブル | 変更前後の値・日時・変更者を完全に追跡する必要がある。論理削除では不十分 |

**解説：** 削除戦略はシステム要件（復元の必要性・監査要件・参照整合性）によって決まります。「後から変えたくなった」はよくある失敗パターンなので、設計時にチームで方針を統一しておくことが重要です。
