...
> 参照：[1. 主キーの概念と必要性](#1-主キーprimary-keyの概念と必要性)

以下のテーブルを設計する際、主キーとして何を使うべきか理由とともに答えてください。

> **社員テーブル**：社員番号（例: EMP-001）、氏名、部署、メールアドレスがある

<details>
<summary>回答を見る</summary>

**推奨：BIGSERIAL（代理キー）を主キーにし、社員番号には UNIQUE 制約を付ける**

```sql
CREATE TABLE employees (
  id          BIGSERIAL PRIMARY KEY,
  emp_code    TEXT UNIQUE NOT NULL,  -- 社員番号
  name        TEXT NOT NULL,
...
**解説：** 社員番号は業務上の識別子（自然キー）ですが、以下の理由で主キーには向きません。
- 番号体系が変わったとき（M&A・部署再編）に全レコードを更新することになる
- 外部テーブルからの外部キー参照が文字列になり効率が落ちる

代理キー（BIGSERIAL）を主キーにし、社員番号は UNIQUE で一意性を保証するのが現場の標準パターンです。

</details>

### 問題2: 外部キーの定義

> 参照：[3. 外部キーの概念](#3-外部キーforeign-keyの概念) ・ [4. ON DELETE / ON UPDATE の挙動](#4-on-delete-on-update-の挙動)

以下の要件を満たすテーブル定義を書いてください。

- `customers` テーブル（id, name）
- `orders` テーブル（id, customer_id, amount）
- 顧客が削除されたら注文も一緒に削除する
...
**解説：** `REFERENCES customers(id)` で外部キー制約を設定します。`ON DELETE CASCADE` は参照先（customers）の行が削除されたとき、参照元（orders）の行も自動的に削除します。要件に応じて `ON DELETE RESTRICT`（削除を防ぐ）や `ON DELETE SET NULL`（NULLにする）を選択します。

</details>

### 問題3: 多対多の設計

> 参照：[5. 1対多・多対多の関係の概念](#5-1対多・多対多の関係の概念)

`students` と `courses` の多対多関係を中間テーブルで実現してください。また、同じ学生が同じ講座に重複して登録できないよう制約を付けてください。
...
**解説：** 中間テーブル `student_courses` に両テーブルへの外部キーを持たせます。複合主キー `PRIMARY KEY (student_id, course_id)` が重複登録防止の UNIQUE 制約を兼ねます。`enrolled_at` のように「関係の属性」を中間テーブルに追加できるのが中間テーブルの強みです。
